...
| 인증 | JWT (Redis 캐시 + DB 저장) | 카카오 + 네이버 + Google + Apple(iOS) + 이메일 로그인, 동일 이메일 계정 통합 |
...
### Haptic Feedback
- 적용 대상: 좋아요, 찜(북마크), 폼 "다음" 버튼, 상담 요청 제출 버튼
- `HapticFeedback.lightImpact()`: 좋아요/찜 토글
- `HapticFeedback.mediumImpact()`: 상담 요청 제출 완료
- Flutter 내장 API 사용, 별도 패키지 불필요
...
### 6.3 찜(북마크) 기능
- 병원 찜: 병원 상세에서 "찜" 토글 (POST/DELETE /bookmarks, targetType: HOSPITAL)
- 게시글 찜: 커뮤니티 게시글 "찜" 토글 (POST/DELETE /bookmarks, targetType: POST)
- 찜 목록: 병원/게시글 탭 분리, 마이페이지에서 접근
- 병원 비교: 찜한 병원 복수 선택 → 비교 화면 (전문분야, 평점, 리뷰 수, 거리, 응답시간 나란히 비교)

### 6.4 상담 상태 실시간 피드백
- 4단계 프로세스 바: 접수완료 → 검토중 → 선정중 → 매칭완료
- 시간 경과에 따른 단계별 정직한 메시지 (접수 → 확인 중 → 병원 탐색 중 → 매칭 완료)
- 매칭 대기 중 스켈레톤 카드(shimmer) → 실제 카드 전환 애니메이션
- Centrifugo WS + FCM 푸시로 상태 업데이트
...
- 같은 글이 매일 ���른 사진으로 노출 → 신선함 유지
- 이미지 2장 이상 게시글: 우측 상단 dot 인디케이터로 "사진 더 있음" 힌트

### 6.8 촬영 가이드 오버레이
- 상담 요청 사진 촬영 시 반투명 얼굴 윤곽 오버레이 (정면/좌측면/우측면 3종)
- 마이크로카피: "정면을 바라보고 얼굴을 가이드에 맞춰주세요"
- 안심 문구 상시 표시: "사진은 매칭된 병원만 볼 수 있으며, 상담 종료 후 90일 뒤 자동 삭제됩니다"

### 6.9 햅틱 피드백
- 적용 대상: 좋아요, 찜, 폼 다음 버튼, 상담 요청 제출
- lightImpact: 좋아요/찜 토글
- mediumImpact: 상담 요청 제출 완료

### 6.10 스텝퍼 응원 메시지
- 상담 요청 폼 5단계 중 4~5단계에만 배치 (1~3단계는 없음, 피로감 방지)
- 4단계: "거의 다 됐어요" / 5단계: "마지막 단계예요"
...
## 10. IA (Information Architecture) 페이지 넘버링
