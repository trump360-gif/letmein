...
import { Eye, ThumbsUp, MessageSquare, PenSquare, Home, ChevronRight, TrendingUp, ImageOff } from 'lucide-react'
...
export async function BoardCardSkin({ board, posts, pagination }: BoardSkinProps) {
          ...
          {posts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {posts.map((post) => (
                ...
                >
                  {/* 썸네일 */}
                  <div className="relative h-[160px] overflow-hidden bg-[#F5F5F5] sm:h-[150px]">
                    {post.thumbnailUrl ? (
                      <img
                  ...
                  <div className="flex flex-col gap-2 p-3">
                    <h3 className="line-clamp-2 text-[14px] font-semibold leading-snug text-[#0A0A0A]">
                      {post.title}
                    </h3>
                    {post.excerpt && (
                      <p className="line-clamp-2 text-[12px] leading-relaxed text-[#777777]">
                        {post.excerpt}
                      </p>
                    )}
                    <div className="mt-1 flex items-center justify-between text-[11px] text-[#AAAAAA]">
