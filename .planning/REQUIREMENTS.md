...
- [ ] **HAUTH-01**: 병원 전용 로그인 페이지 (/hospital-login)
- [ ] **HAUTH-02**: 로그인 후 역할(admin/hospital)에 따라 다른 사이드바/대시보드 표시
- [ ] **HAUTH-03**: 병원 역할은 hospital 전용 라우트만 접근 가능 (admin 라우트 차단)

### 병원 대시보드

- [ ] **HDASH-01**: 대시보드 메인 — 신규 매칭 수, 활성 채팅 수, 평균 응답률, 평점 표시
- [ ] **HDASH-02**: 대시보드에서 최근 상담 요청 3건 미리보기
- [ ] **HDASH-03**: 대시보드에서 최근 리뷰 3건 미리보기
...
- [ ] **HDOC-01**: 의료진 목록 조회
- [ ] **HDOC-02**: 의료진 추가 (이름, 전문분야, 경력)
- [ ] **HDOC-03**: 의료진 수정/삭제
- [ ] **HDOC-04**: 의료진 순서 드래그앤드롭 변경

### 병원 상담 관리

- [ ] **HCONS-01**: 매칭된 상담 요청 목록 조회 (상태별 탭 필터)
- [ ] **HCONS-02**: 상담 요청 상세 확인 (카테고리, 부위, 시기, 사진, 메모)
- [ ] **HCONS-03**: 상담 응답 작성 (소개 60자 + 경험 60자 + 메시지 10~3000자)
- [ ] **HCONS-04**: 응답 발송 후 상태 변경 확인

### 병원 채팅

- [ ] **HCHAT-01**: 병원 채팅방 목록 조회 (안읽음 수, 마지막 메시지, 시간)
- [ ] **HCHAT-02**: 채팅방 메시지 실시간 송수신 (Centrifugo WebSocket 또는 polling)
- [ ] **HCHAT-03**: 방문 예약 카드 발송 (날짜/시간/메모)
- [ ] **HCHAT-04**: 유저 수락/거절 상태 확인
- [ ] **HCHAT-05**: 면책 고지 배너 상단 고정

### 병원 광고 관리

- [ ] **HAD-01**: 광고 크레딧 잔액 확인
- [ ] **HAD-02**: 광고 크리에이티브 등록 (이미지 + 헤드라인)
- [ ] **HAD-03**: 광고 캠페인 생성 (기간, 일 예산, 크리에이티브 선택)
- [ ] **HAD-04**: 캠페인 목록 + 성과 리포트 (노출, 클릭, 소비)
- [ ] **HAD-05**: 캠페인 일시정지/재개

### 병원 프리미엄

- [ ] **HPREM-01**: 현재 구독 상태 확인 (티어, 만료일)
- [ ] **HPREM-02**: 프리미엄 혜택 안내 페이지
